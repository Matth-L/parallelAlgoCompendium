...
int main(int argc, char *argv[]) {
    ...
    // Création de la fenêtre MPI
    int *win_buffer = (rank == 0) ? (int *)malloc(size * sizeof(int)) : NULL; // Buffer pour le processus root
    MPI_Win win;
    MPI_Win_create(win_buffer, size * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win);

    // Stocker le résultat local dans la mémoire exposée du processus root
    MPI_Win_fence(0, win); // Synchronisation initiale
    MPI_Put(&local_count, 1, MPI_INT, 0, rank, 1, MPI_INT, win);
    MPI_Win_fence(0, win); // Synchronisation après l'opération

    // Le processus root agrège les résultats
    if (rank == 0) {
        int global_count = 0;
        for (int i = 0; i < size; i++) {
            global_count += win_buffer[i];
        }
